...
def createCrosswordCSP(cw):
    ...
    for key in cw.letters.keys():
        csp.add_variable(key, cw.letters[key].domain)
    ...
    for i,key1 in enumerate(cw.words.keys()):
        word = cw.words[key1]
        csp.add_variable(key1, word.domain)
        ...
        for idx in range(word.length):
            ...
            csp.add_binary_factor(key1, letterKey, lambda x,y: x[idx] == y)
